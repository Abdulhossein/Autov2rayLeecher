...
def get_v2ray_links(url):
    ...
    if response.status_code == 200:
        ...
        # Use regex to extract V2Ray links
        v2ray_configs = extract_v2ray_links(all_text)
        return v2ray_configs
    else:
        print(f"Failed to fetch URL (Status Code: {response.status_code})")
        return None
def extract_v2ray_links(text):
    # Regular expression pattern to match V2Ray links
    pattern = r'(vless://[^\s]+|ss://[^\s]+|trojan://[^\s]+|tuic://[^\s]+)'
...
def create_sub_section():
    ...
    with open(readme_path, 'w', encoding='utf-8') as readme_file:
        if found_sub_section:
            readme_file.write(content.replace(content[content.find(
